function drawHive(x, y, radius, slots, level, mutations, beequips) {
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    if (slots[i] != 'U') {
      ...
      try {
        ...
        if (lvl) {
          let mut = mutationColors[mutations[i]] || color(255);
          console.log(mut)
          displayLevel(hexes[hexes.length - 1].x, hexes[hexes.length - 1].y, lvl, mut);
        }
      } catch(error){
      }
    }
...
function displayLevel(x, y, number, col = color(255)) {
  let posX = x-15;
  let posY = y;
  textFont(hwfnt);
